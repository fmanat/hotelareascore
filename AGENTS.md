...
> This project is picked up across sessions by different AI coding agents
> (Claude Code reads `CLAUDE.md`, Codex reads `AGENTS.md` — kept identical,
> same operating rules regardless of which agent is driving). Whichever
> agent you are: `docs/STATE.md` is the single source of truth for current
> phase, decisions, and blockers — it overrides any stale assumption from a
> prior session or from this file's own examples.
